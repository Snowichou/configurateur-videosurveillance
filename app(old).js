...
(() => {
  ...
  const DOM = {
    ...
    let btnExportPdf = null;
    let LAST_PROJECT = null;

    // ✅ petit cache projet (évite recalcul x fois dans un même render)
  ...
  function renderFinalSummary(proj) {
    const cams = MODEL.cameraLines
      .map((l) => {
        ...
        if (!cam) return null;
        return `• ${l.qty} × ${safeHtml(cam.name)} (${safeHtml(l.quality || "standard")})`;
      })
      .filter(Boolean)
      .join("<br>");

    const accs = (MODEL.accessoryLines || [])
      .map((l) => `• ${l.qty} × ${safeHtml(l.name || l.accessoryId)}`)
      .filter(Boolean)
      .join("<br>");

    const sw = proj.switches.required
      ? proj.switches.plan
          .map((p) => `• ${p.qty} × ${safeHtml(p.item.id ? `${p.item.id} — ${p.item.name}` : p.item.name)} (${p.item.poe_ports} ports)`)
          .join("<br>")
      : "• (non obligatoire)";

    const nvr = proj.nvrPick.nvr;
    const nvrLine = nvr ? `${safeHtml(nvr.name)} (${nvr.channels} canaux, ${nvr.max_in_mbps} Mbps)` : "Aucun";

    const disk = proj.disks;
    const hddLine = disk
      ? `${disk.count} × ${disk.sizeTB} TB (total ${disk.totalTB} TB)${disk.hddRef ? ` • ${safeHtml(disk.hddRef.id)} — ${safeHtml(disk.hddRef.name || "")}` : ""}`
      : "Aucun";
      ...
      <div class="recoCard">
        <div class="recoHeader">
          <div>
            ...
            <div class="muted">Matériel recommandé + calculs (MVP)</div>
          </div>
          <div class="score">✅</div>
        </div>
        ...
        <div class="reasons">
          ...
          <strong>NVR :</strong><br>• ${nvrLine}<br><br>
          <strong>Switch PoE :</strong><br>${sw}<br><br>
          <strong>Stockage :</strong><br>• ${safeHtml(hddLine)}<br><br>
  ...
  function setFinalContent(proj) {
    ...
    DOM.alternativesEl.innerHTML = `<div class="muted">—</div>`;
  ...
  if (!allowed) MODEL.ui.resultsShown = false;
  ...
  const showCol = allowed && MODEL.ui.resultsShown;

  // ✅ ✅ ✅ ICI : on (re)récupère le container des actions
  const headerActions = document.querySelector(".actions");

  // ✅ Bouton PDF PRO (stable)
  if (!btnExportPdf) {
    btnExportPdf = document.createElement("button");
    btnExportPdf.id = "btnExportPdf";
    btnExportPdf.type = "button";
    btnExportPdf.className = "btn secondary";
    btnExportPdf.textContent = "Exporter PDF (pro)";
    btnExportPdf.title = "Génère un PDF structuré (rapport).";
    btnExportPdf.addEventListener("click", exportProjectPdfPro);
  }

  // ✅ si le bouton existe mais n'est plus dans le DOM, on le ré-attache
  if (headerActions && !headerActions.contains(btnExportPdf)) {
  ...
  function updateProgress() {
    ...
    DOM.progressText.textContent = `Étape ${MODEL.stepIndex + 1}/${STEPS.length} • ${pct}%`;
  }
  ...
  _renderProjectCache = null; // ✅ reset cache pour ce render
    ...
    if (step.id === "mounts") body.innerHTML = renderStepAccessories();
    if (step.id === "nvr_network") body.innerHTML = renderStepNvrNetwork();
    if (step.id === "storage") body.innerHTML = renderStepStorage();
  ...
  // ==================================================
  // 3) Paramètres d’enregistrement (commit + render)
  // ==================================================
  if (action === "recDays") {
    MODEL.recording.daysRetention = clampInt(el.value, 1, 365);
    render();
    return;
  }

  if (action === "recHours") {
    MODEL.recording.hoursPerDay = clampInt(el.value, 1, 24);
    render();
    return;
  }

  if (action === "recOver") {
    MODEL.recording.overheadPct = clampInt(el.value, 0, 100);
    render();
    return;
  }

  if (action === "recReserve") {
    MODEL.recording.reservePortsPct = clampInt(el.value, 0, 50);
    render();
    return;
  }

  if (action === "recFps") {
    MODEL.recording.fps = parseInt(el.value, 10);
    render();
    return;
  }

  if (action === "recCodec") {
    MODEL.recording.codec = el.value;
    render();
    return;
  }

  if (action === "recMode") {
    MODEL.recording.mode = el.value;
    render();
    return;
  }
}
  ...
  if (action === "inputBlockField") {
    ...
    // ✅ Stock brut pendant la frappe (anti jump)
    // mais on normalise un minimum distance_m (optionnel)
    const v = el.value;

    if (field === "distance_m") {
      // on garde seulement les chiffres pendant la saisie
      // (évite les "e", "-", etc. sur certains navigateurs)
  ...
  if (action === "inputBlockQty") {
    ...
    // ✅ Stock brut pendant saisie (string) pour éviter le “jump”
    // mais on filtre pour garder seulement digits
  ...
  // =========================
  // ✅ Stock brut pendant saisie (pas de clamp ici)
  // Le clamp + render est fait dans onStepsChange
  ...
  function buildExportObject() {
    ...
    return {
      ...
      catalogStats: {
        ...
        nvrs: CATALOG.NVRS.length,
        hdds: CATALOG.HDDS.length,
        switches: CATALOG.SWITCHES.length,
        accessories: CATALOG.ACCESSORIES_MAP.size,
      },
      output: proj,
    };
  }
  ...
  function canGoNext() {
    ...
    if (step === "cameras") {
      return MODEL.cameraLines.some((l) => l.cameraId && (l.qty || 0) > 0);
    }
    return true;
  }
  ...
  DOM.btnExportJson.addEventListener("click", () => {
    ...
    DOM.exportArea.value = JSON.stringify(obj, null, 2);
  });

  DOM.btnExportCsv.addEventListener("click", () => {
    const obj = buildExportObject();
    DOM.exportArea.value = toCsv(obj);
  });

  

  // Delegation sur #steps (1 seul set de listeners)
  DOM.stepsEl.addEventListener("click", onStepsClick);
  DOM.stepsEl.addEventListener("change", onStepsChange);
  DOM.stepsEl.addEventListener("input", onStepsInput);
